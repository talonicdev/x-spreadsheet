...
export default class DataProxy {
  ...
  pasteFromSystemClipboard(resetSheet, eventTrigger, evt) {
    function insert(text,that) {
      const contentToPaste = that.parseClipboardContent(text);
      let startRow = selector.ri;
      contentToPaste.forEach((row) => {
        let startColumn = selector.ci;
        row.forEach((cellContent) => {
          that.setCellText(startRow, startColumn, cellContent, 'input');
          startColumn += 1;
        });
        startRow += 1;
      });
      resetSheet();
      eventTrigger(that.rows.getData());
    }
    const { selector } = this;
    if (navigator.clipboard.readText) {
      ...
      if (evt && evt.clipboardData) {
        insert(evt.clipboardData.getData("text"),this);
      }
    }
  }
